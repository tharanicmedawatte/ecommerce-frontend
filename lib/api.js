...
export async function createPaymentIntent(token) {
  return apiFetch(`${API_V1}/orders/create-payment-intent`, { method: 'POST' }, token);
}

export async function getOrderHistory(token) {
  return apiFetch(`${API_V1}/orders/history`, {}, token);
}

export async function getOrder(token, orderId) {
  return apiFetch(`${API_V1}/orders/${orderId}`, {}, token);
}
